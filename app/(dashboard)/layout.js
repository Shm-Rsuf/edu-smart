...
const DashboardLayout = ({ children }) => {
  ...
  return (
    <div className="mx-auto min-h-screen flex">
      <div
        className={`left-side col-span-1 bg-[#092A67] min-h-screen transition-all duration-300 ${isToggleSidebar ? 'w-[70px]' : 'w-[216px]'}`}
      >
        <div className="sticky top-0 left-0 bg-[#092A67] h-20 flex justify-center items-center border-b-[1px] border-gray-200 z-10">
        { !isToggleSidebar ? <span className="text-white font-bold text-lg">
            <span className="text-lime-400">Edu</span>Smart
          </span> :  <span className="text-lime-400 font-bold">Edu</span> }
        </div>
